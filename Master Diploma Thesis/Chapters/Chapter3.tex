...
\subsection{Definition of NOCGPROX Variables}
    ...
    \begin{equation}\label{eq:3.1.5}
        \begin{align}
            ...
            & - \left\beta \sum_{j \in \mathcal{N}(i)} \sum_{\ell = 1}^{\lfloor t/DT \rfloor} \left( x_i \left (\ell \left \lfloor \frac{t}{T} \right \rfloor T \right) - x_j \left ( \ell \left  \lfloor \frac{t}{T} \right \rfloor T \right) \right) \right]
        \end{align}
    \end{equation}
...
\subsection{Gradient Based Regulation Algorithm}
    ...
    \begin{equation}\label{eq:3.1.8}
        \begin{align}
            q_i^{(\nu + 1)D} = x_i^{(\nu + 1)D} - x_i^{\nu D} + \frac{1}{2\beta d_i}& \left[ \beta \sum_{j \in \mathcal{N}(i)} x_j^{\nu D}  - \nabla f_i \left( x_i^{\nu D}\right) \right\\
            & - \left\beta \sum_{j \in \mathcal{N}(i)} \sum_{\ell = 1}^{\lfloor \nu/D \rfloor} \left( x_i^{\ell + \nu D} - x_j^{\ell + \nu D}\right) \right], i = \{1,...,N\}
        \end{align}
    \end{equation}
    ...
    \begin{equation}\label{eq:3.1.10}
        \mu^{(\nu D + \ell)} = \left \{ \begin{matrix}
            ...
            \mu^{\nu D} + \beta \sum_{\ell = 1}^{\lfloor \nu/D \rfloor} Ax^{\ell D}, \hspace{5.0mm} \ell = D 
        \end{matrix} \right
    \end{equation}
    ...
    \begin{equation}\label{eq:3.1.11}
        q^{(\nu + 1)D} = x^{(\nu + 1)D} - x^{\nu D} + \frac{1}{2\beta} \mathrm{D}^{-1}\left[\beta L_+x^{\nu D} - \nabla f(x^{\nu D}) - \beta A^T\mu^{\nu D}\right]
    \end{equation}
    ...
    \begin{algorithm}[H]
        ...
        \textbf{Input}: $\mu^0 = 0$, $x^0 \in \mathbf{R}^Q$, $q^{(\nu +1)D} \in \mathbf{R}^Q$, $D, T, d_{max}, \beta, \varepsilon$\\
        \textbf{Output}: $x^{(\nu +1)D} \in \mathbf{R}^Q$, $s_i^{\nu D} \in \mathbf{R}^Q$
        \begin{algorithmic}[1]
            \State $\nu \gets 1$;
            \State $\ell \gets 1$;\\
            \While {$||\nabla f(x)|| \leq \varepsilon$}{
                \For {$i = 1:N$}{
                    ...
                    \For{$j \in \mathcal{N}(i)$}{
                        $s_i^{\nu D}$ = $\frac{1}{2\beta d_i} \left[\beta x_j^{\nu D} - \nabla f_i(x_i^{\nu D}) - \beta \mu_i^{\nu D + \ell}\right]$;
...
\section{Convergence Analysis}
    ...
    \begin{proof}
        ...
        \end{equation*}
        \begin{equation*}
            \Rightarrow \beta > 4\mathrm{L} + \epsilon +c(\mathrm{L} + \epsilon) + \frac{6\mathrm{L}^2}{\beta\underline{\sigma}^2(A)} + c\mathrm{L}
        \end{equation*}
        \begin{equation*}
           \Rightarrow \beta^2 -(2\mathrm{L}(2+c) + (1+c)\epsilon)\beta - \frac{6\mathrm{L}^2}{\underline{\sigma}^2(A)} > 0
        \end{equation*}
        ...
        \begin{equation}\label{eq:3.2.22}
            \beta > \frac{1}{2}\left(2\mathrm{L}\left(2+c\right) + \left(1+c\right)\epsilon + \sqrt{\left(\left(2\mathrm{L}\left(2+c\right) + \left(1+c\right)\epsilon\right)\right)^2 + \frac{24\mathrm{L}^2}{\underline{\sigma}^2(A)}} \right)
        \end{equation}
    \end{proof}
